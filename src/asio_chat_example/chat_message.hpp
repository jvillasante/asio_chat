...
public:
  ...
  std::size_t length() const { return header_length + body_length_ + from_length_ + to_length_; }
  ...
  void from_length(std::size_t new_length) {
    ...
    if (from_length_ > max_from_length) from_length_ = max_from_length;
  }
  ...
  void to_length(std::size_t new_length) {
    ...
    if (to_length_ > max_to_length) to_length_ = max_to_length;
  }

  void pack() {
    memset(data_, 0, header_length + max_from_length + max_to_length + max_body_length);
    char header[header_length + 1] = "";
    std::sprintf(header, "%4d%4d%4d", static_cast<int>(from_length_), static_cast<int>(to_length_),
  ...
  bool unpack() {
    ...
    std::strncat(to_length_buffer, data_ + header_from_length, header_to_length);
    ...
    std::strncat(body_length_buffer, data_ + header_from_length + header_to_length, header_body_length);
